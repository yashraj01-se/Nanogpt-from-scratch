...
class MultiHeadAttention(nn.Module):
    ...
    #The attention output projection belongs inside the attention sublayer and must be applied before the residual connection so the residual operates in the model’s embedding space.
   
    def forward(self, x):
        out = torch.cat([h(x) for h in self.heads], dim=-1)
        out = self.project(out)
        #x = x + Projection(Concat(Heads(x)))
        return out
...
class FeedForward(nn.Module):
    ...
    def __init__(self, n_embd):
        ...
        self.net = nn.Sequential(
            nn.Linear(n_embd,4*n_embd), # Expand to 4 times the embedding dimension because larger hidden layers help model complex patterns.
            nn.ReLU(),
            nn.Linear(4*n_embd,n_embd) # Projection back to n_embd dimension(Residual pathway)
        )
...
class Block(nn.Module):
    ...
    """Multiple attention heads are computed in parallel, their outputs are concatenated into a single representation, projected back into the model’s embedding space, and then combined with the previous representation via a residual connection."""
    def __init__(self, n_embd, n_head):
        super().__init__()
        self.sa = MultiHeadAttention(num_heads=n_head, n_embd=n_embd, block_size=block_size)
        self.ffwd = FeedForward(n_embd)
        self.ln1 = nn.LayerNorm(n_embd) #LayerNorm stabilizes the distribution of activations before each sublayer, which indirectly stabilizes gradient flow during backpropagation.
        self.ln2 = nn.LayerNorm(n_embd)

    def forward(self, x):
        x = x + self.sa(self.ln1(x))
        x = x + self.ffwd(self.ln2(x))
        #Multi-head attention combines parallel attention outputs into a single representation, projects it back into the embedding space, and integrates it with the previous representation using a residual connection.
        return x
